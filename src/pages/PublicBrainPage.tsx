...
export default function PublicBrainPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`http://localhost:3000/api/v1/brain/${shareLink}`);
      ...
      <main className="max-w-5xl mx-auto p-6">
        {isLoading ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-[180px] rounded-lg" />)}
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center py-20 text-center">
            <p className="text-muted-foreground">This brain has no content yet</p>
          </div>
        )}
      </main>
